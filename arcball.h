...
//求取三维向量之间的夹角(弧度)
static float Vector3fAng(const Vector3fT* NewObj, const Vector3fT* v1)
{
    assert(NewObj && v1);

    float fenzi=NewObj->s.X * v1->s.X+NewObj->s.Y * v1->s.Y +NewObj->s.Z * v1->s.Z;
    double jiaodu=acos(fenzi/(sqrt(NewObj->s.X*NewObj->s.X+NewObj->s.Y*NewObj->s.Y+NewObj->s.Z*NewObj->s.Z)*sqrt(v1->s.X*v1->s.X+v1->s.Y*v1->s.Y+v1->s.Z*v1->s.Z)));
    return jiaodu;
}
//3维点的长度的平方
inline
